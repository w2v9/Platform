...
export async function getLeaderboardData(timeFilter: TimeFilter = TimeFilter.ALL_TIME): Promise<{
    ...
    try {
        ...
        try {
            ...
            console.error("Error fetching reports, falling back to user-specific reports:", err);
        ...
        usersSnapshot.forEach((doc) => {
            ...
            if (quizResults && quizResults.length > 0) {
                ...
                const maxPossibleScore = quizResults.reduce((sum: number, q: QuizReport) => sum + (q.maxScore || 0), 0);
                const actualTotalScore = quizResults.reduce((sum: number, q: QuizReport) => sum + (q.score || 0), 0);

                const completionTimes = quizResults.map((q: QuizReport) => q.timeTaken || 0);
                const totalCompletionTime = completionTimes.reduce((sum: number, time: number) => sum + time, 0);
        ...
        const stats: LeaderboardStats = {
            ...
            totalQuizzes,
            averageScore: totalScoreCount > 0 ? Math.round((totalScoreSum / totalScoreCount) * 100) / 100 : 0,
        ...
        try {
            ...
            if (userReports.length === 0) {
                return {
                    leaderboard: [],
                    stats: { totalUsers: 0, totalQuizzes: 0, averageScore: 0, topScore: 0 }
                };
            }
            ...
            const leaderboardEntry: LeaderboardEntry = {
                ...
                averageScore: Math.round(averageScore * 100) / 100,
                totalScore: userReports.reduce((sum: number, q: QuizReport) => sum + (q.score || 0), 0),
                maxScore: userReports.reduce((sum: number, q: QuizReport) => sum + (q.maxScore || 0), 0),
                bestScore: Math.round(bestScore * 100) / 100,
                totalCompletionTime: userReports.reduce((sum: number, q: QuizReport) => sum + (q.timeTaken || 0), 0),
                averageCompletionTime: Math.round(userReports.reduce((sum: number, q: QuizReport) => sum + (q.timeTaken || 0), 0) / userReports.length),
...
function calculateBadges(user: User, quizResults: QuizReport[]): string[] {
    ...
    const perfectScores = validScores.filter((score: number) => score === 100).length;
    ...
    if (validTimes.length > 0) {
        const avgCompletionTime = validTimes.reduce((sum: number, time: number) => sum + time, 0) / validTimes.length;
        if (avgCompletionTime < 120) { // Less than 2 minutes average
...
export async function getTopPerformers(limit_count: number = 10): Promise<LeaderboardEntry[]> {
    try {
        const { leaderboard } = await getLeaderboardData();
...
export async function getQuizLeaderboard(
    ...
    try {
        ...
        try {
            ...
            console.error("Permission error getting all quiz reports, falling back to user's reports only:", err);
...
export async function getUserQuizAttempts(
    ...
    try {
        ...
        if (!isOwnAttempts && !isAdmin) {
            ...
            throw new Error("You don't have permission to view other users' attempts");
        }
        ...
        console.error(`Error fetching attempts for user ${userId} on quiz ${quizId}:`, error);
        throw error;
    }
}
