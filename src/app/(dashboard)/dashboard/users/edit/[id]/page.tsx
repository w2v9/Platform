...
export default function EditUserPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const toggleQuizAccess = async (quizId: string, hasAccess: boolean) => {
        if (!user) return;

        const loadingToast = toast.loading(
            hasAccess
                ? "Removing quiz access..."
        ...
        try {
            ...
            if (hasAccess) {
                const updatedQuizAccess = { ...user.quizAccess };
                delete updatedQuizAccess[quizId];

                await updateDoc(userRef, {
                    quizAccess: updatedQuizAccess,
                    "metadata.updatedAt": new Date().toISOString()
                });
                ...
                toast.success("Quiz access removed successfully", {
                    id: loadingToast,
                });
                await recordLog({
                    id: v4(),
                    ...
                    details: JSON.stringify({
                        updatedUser: user.id,
                        accessRemoved: quizId,
                    }),
                    timestamp: new Date().toISOString(),
                });

            } else {
                const updatedQuizAccess = {
                    ...user.quizAccess,
                    [quizId]: {
                ...
                toast.success("Quiz access granted successfully", {
                    id: loadingToast,
                });
                await recordLog({
                    id: v4(),
            ...
            let errorMessage = "Failed to update quiz access";
            ...
            toast.error("Failed to update quiz access", {
                id: loadingToast,
            });
        }
    };
            ...
            <div className="p-4">
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <TabsContent value="quizzes">
                        <Card>
                            <CardHeader>
                                <CardTitle>Quiz Access Management</CardTitle>
                                <CardDescription>
                                    Control which quizzes this user can access
                                </CardDescription>
                            </CardHeader>
                            <CardContent>
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {availableQuizzes.map((quiz) => {
                                                const hasAccess = user?.quizAccess?.[quiz.id];
                                                return (
                                                        ...
                                                        <TableCell>
                                                            <Switch
                                                                ...
                                                                onCheckedChange={(checked) =>
                                                                    toggleQuizAccess(quiz.id, !!hasAccess)
                                                                }
                                                            />
                                                        </TableCell>
                                                        <TableCell>
                                                            {hasAccess
                                                                ? new Date(hasAccess.grantedAt).toLocaleDateString()
                                                                : "-"}
                                                        </TableCell>
                                                        <TableCell>
                                                            {hasAccess?.expiresAt
                                                                ? new Date(hasAccess.expiresAt).toLocaleDateString()
                                                                : "Never"}
                                                        </TableCell>
                                                        <TableCell>
                                                            {hasAccess && (
                                                                <div className="flex gap-2">
                                                                    <Button
                                                                        ...
                                                                        onClick={() => {
                                                                            ...
                                                                            const userRef = doc(db, "users", user.id);
                                                                            updateDoc(userRef, {
                                                                                [`quizAccess.${quiz.id}.expiresAt`]: twoWeeksFromNow.toISOString(),
                                                                            });

                                                                            // Update local state
                                                                            const updatedUser = { ...user };
                                                                            if (!updatedUser.quizAccess) {
                                                                                updatedUser.quizAccess = {};
                                                                            }
                                                                            updatedUser.quizAccess[quiz.id] = {
                                                                                ...updatedUser.quizAccess[quiz.id],
                                                                    ...
                                                                    <Button
                                                                        ...
                                                                        onClick={() => {
                                                                            ...
                                                                            const userRef = doc(db, "users", user.id);
                                                                            updateDoc(userRef, {
                                                                                [`quizAccess.${quiz.id}.expiresAt`]: null,
                                                                            });

                                                                            // Update local state
                                                                            const updatedUser = { ...user };
                                                                            if (updatedUser.quizAccess?.[quiz.id]) {
                                                                                delete updatedUser.quizAccess[quiz.id].expiresAt;
                                                                            }
